...
class VideoClsDataset(Dataset):
    ...
    def __init__(self,
        ...
        # Check if in compositional mode (expects 5 columns: path, verb, manip_obj, target_obj, tool)
        self.compositional_mode = getattr(args, 'compositional_mode', False) if args is not None else False
        if self.compositional_mode and cleaned.shape[1] >= 5:
            # Read compositional labels: (verb, manip_obj, target_obj, tool)
            self.label_array = list(zip(cleaned.values[:, 1], cleaned.values[:, 2], 
                                       cleaned.values[:, 3], cleaned.values[:, 4]))
        else:
